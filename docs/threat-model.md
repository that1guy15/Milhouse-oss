...
|---|---|---|---|---|
| Provider/file/webhook content contains secrets, PII, prompt injection, or oversized input | Strict source models, allowlists, byte/page/record bounds, trust/privacy class, redaction before persistence | Safe reason codes, redaction counts, schema-drift/health events | Drop restricted value; quarantine only already-redacted conflict metadata | Adversarial nested/encoded/Unicode/markup/injection/size corpus |
...
| Malicious third-party plugin | No auto-install, exact distribution/version/group/entry-point allowlist; honest trusted-code model | Load failure isolation and package metadata in health | Disable/remove plugin and rotate affected credentials; no sandbox claim | Invalid/missing/malicious-entry failure isolation and provenance checks |
| External notification/action leaks or duplicates | Disabled default, classification allowlist, egress redaction, preview/confirm, least-privilege token, idempotent marker | Delivery/audit state without secret-bearing URL/body | Disable sink, rotate token, retry safely or hold | Markdown injection, redaction, retry/rate/duplicate tests |
...
| Dependency/workflow/artifact compromise | Locked dependencies, full-SHA Actions, read-only workflow permissions, scans, build once, SBOM/provenance | Dependabot, dependency review, CodeQL, gitleaks, artifact inventory | Hold/yank release, advisory, patch from reviewed source | Fork/no-secret CI, planted secret, provenance/SBOM verification |
