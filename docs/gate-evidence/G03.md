...
tree `6d89f6b`) returned FAIL with four P1 blockers and one P2 on 2026-07-31, so G03 is NOT a valid
candidate and MUST NOT be accepted.** The open findings are recorded below; the earlier claim that
each G03 assertion had passing evidence is **withdrawn** until they are remediated on a new
production SHA and a fresh independent full-SHA review passes. In particular, the collision-safety and
compaction audit-lineage assertions do **not** currently hold, and the same-blockers exact-head FAIL
on PR #72's head `9128fad` (whose tree is identical to the merged `ced5513`) means the prior
"remediated" language was overstated.
...
- **[P2] The packet is not bound to final exact-head evidence** — the recorded `4644 passed` predates
  the fourteen added cases; the packet must be refreshed with the exact tested SHA/tree, environment,
  coverage totals, and effective replay count on the remediated head.
...
**G03 is NOT an acceptable candidate.** An independent gate review of the integrated head
(`main` `b2822f3`, tree `6d89f6b`; equivalently PR #72 head `9128fad`) returned FAIL with four P1
blockers and one P2 on 2026-07-31 (recorded above). The earlier claim that each G03 assertion had
passing local evidence — specifically collision-safe compaction and the compaction audit lineage — is
**withdrawn**. The deterministic-collision and keyed-lineage assertions do not currently hold, and the
"deferred to W06" note does not discharge the plan §4.7 lineage deliverable. W03 and G03 are **In
progress**. Coverage and the crash/concurrency/failure-injection harness and the 10,000-record replay
proof remain in place, but the packet as a whole cannot be certified until the four P1 findings are
remediated on a new protected-`main` SHA and a fresh independent full-SHA review passes. Acceptance,
when it is eventually granted, additionally requires supported-host physical-crash evidence, and must
bind to the exact protected-`main` commit that merges the remediated candidate, with a follow-up
ledger entry recording that merge SHA, squash-tree equality, post-merge Required CI, and the
acceptance.
