...
Status: **NOT ready and NOT accepted — the 2026-08-01 remediation was disproven by two independent
reviews.** The merged head (`main` `91b97b0`, tree `fb372e2` = PR #76 head `f9ce189`) FAILED its
exact-head review, and a separate review of the crash-procedure PR surfaced a further code defect:
**PR #76** review ([5151585903](https://github.com/that1guy15/Milhouse-oss/pull/76#issuecomment-5151585903),
2026-08-01, 149 tests) returned **FAIL: 3 P1 (same-survivor/different-exporter collision still
strands; keyed lineage optional/unwired → NULL `_audit` resources; audit omits the mandated old/new
content/file hashes) + 1 P2 (packet not bound to the exact head)**; **PR #74** review
([5148867847](https://github.com/that1guy15/Milhouse-oss/pull/74#issuecomment-5148867847),
2026-08-01) returned **FAIL** including **[P1] a retention crash re-adopts privacy-expired bytes**
(the `_prune_segment` row-before-file delete leaves no durable retirement intent; its source blob is
unchanged in the merged code). **PR #77 posted NO review; its "each finding addressed / candidate
ready / head under review 91b97b0" packet is a later overstatement, now WITHDRAWN.** See D07 for the
accurate, artifact-linked record. **G03 and W03 stay In progress; the defects must be remediated on a
NEW production-`main` SHA with adversarial regression evidence and a fresh independent full-SHA review
before any acceptance. All material below predates D07 and is superseded and disproven by it.**
...
was `91b97b0`. Two independent reviews then FAILED that exact tree (see D07): the **PR #76** review
...
found the plan-mandated old/new hashes still missing; the **PR #74** review
([5148867847](https://github.com/that1guy15/Milhouse-oss/pull/74#issuecomment-5148867847), head
`a2e71b7d`) proved the retention crash re-adopts privacy-expired bytes. Specifically DISPROVEN:
...
review is NOT `91b97b0`.** The first code-remediation attempt (branch `codex/w03-g03-rem2`, PR #79,
head `1b19a91`) was independently reviewed and **FAILED with four P1s**
([5152583787](https://github.com/that1guy15/Milhouse-oss/pull/79#issuecomment-5152583787)); it is
being reworked per owner-approved amendment A07 on a new head, not yet reviewed or merged. The head
under review will be that future reviewed/merged production-`main` SHA. See D07/A07 for the accurate,
artifact-linked record and the required corrections.
...
- **Supported-host physical-crash evidence.** The harness proves the crash boundaries deterministically
  ...
  reproducible in CI.
- **Compaction audit lineage — the earlier "wired into W03" claim is DISPROVEN (see D07).** The
  PR #76 packet claimed `compact_apply` threads the installation `Pseudonymizer` and that a
  maintenance caller loads the key. The PR #76 review found `compact_apply` still defaults
  `pseudonymizer=None` with NO production caller, so production compaction records NULL `_audit`
  lineage, and the audit still omits the plan-mandated old/new content/file hashes. Superseded by
  D07; the fix is pending on PR #79.

## Gate conclusion

**WITHDRAWN / DISPROVEN — see the Status section at the top of this packet and D07.** The prior
conclusion asserted the remediation had merged and that G03 was "READY FOR an independent full-SHA
review of `91b97b0`." Two independent reviews then FAILED that exact tree (D07): PR #76's exact-head
review and PR #74's crash-procedure review. **There is no head currently under review.** W03 and G03
are **In progress**. A code remediation is pending on branch `codex/w03-g03-rem2` (PR #79) — NOT yet
independently reviewed or merged; acceptance requires a fresh independent full-SHA PASS on the future
remediated production-`main` SHA, E06 supported-host physical-crash evidence, and owner acceptance,
recorded then against that SHA.
