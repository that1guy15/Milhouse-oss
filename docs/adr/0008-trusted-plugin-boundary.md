...
Third-party plugins are trusted local Python code running in-process with the Milhouse user's authority. Capability and privacy manifests are review and compatibility declarations, not containment. Milhouse does not auto-install plugins. Third-party discovery/import is disabled unless `[plugins].allow_third_party = true` and an exact allowlist entry matches the raw installed distribution name, installed version, group, and Python `EntryPoint.value` (`module:attribute`). Package-name normalization is used only to locate candidates. Configuration validation inspects only configured path-backed distribution metadata, reads `METADATA`/`PKG-INFO` and `entry_points.txt` under respective 128 KiB and 64 KiB pre-parse byte caps, and fails closed for unsupported metadata backends. It reuses one bounded metadata snapshot per configured distribution in a validation pass. Configured and installed raw versions must each be valid PEP 440, including epoch forms such as `1!2.0`, and must then match exactly rather than by normalized equivalence. Installed entry-point values must contain nonempty dotted Python identifier segments on both sides of `:` and must match exactly. Validation accepts exactly one matching distribution and entry point and never loads plugin code; unlisted, duplicated, mismatched, malformed, oversized, unsupported, or unknown plugins remain unavailable. The W05 runtime registry must revalidate and bind the exact entry-point object it will immediately load. A sandbox claim requires a future separate-process ADR and enforcement design.
...
Every public plugin passes the contract kit and declares supported fixture versions and capabilities. Operators must review and install third-party distributions explicitly. Configuration-time metadata validation proves only an exact local allowlist match at that instant; it does not establish code safety or grant later load authority. A malicious installed plugin is treated as a trusted-code compromise, not a contained input failure.
