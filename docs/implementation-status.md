...
| Active package | W02 G02 evidence correction (defect D02): owner-approved ADR 0016 gate-scope re-scope and [G02.md](gate-evidence/G02.md) rebinding to `b56b698`; owner acceptance of G02 is pending, after which W03 unblocks |
...
| D02 | 2026-07-23 | Gate-scope inconsistency in ADR 0016 revised-validation (owner-approved correction) | ADR 0016's G02 revised-validation required W03/W06/W16-owned filesystem persistence, rotation, recovery, retention, backup, restore, and purge evidence, contradicting the section 4.15 ownership split and creating a G02/W03 gate cycle; the G02 evidence packet also bound its 36-file coverage result to `bb0571d` (35 enforced files) rather than the enforcing commit `b56b698`, and lacked per-slice run/tree traceability. Corrected under plan section 1 change control: the G02 revised-validation is re-scoped to W02-owned evidence with filesystem to G03, CLI/stderr to G06, and backup/restore/purge to G16; [G02.md](gate-evidence/G02.md) is rebound to `b56b698` with a per-slice provenance table. No source code changed; surfaced by the PR #31 owner review |
