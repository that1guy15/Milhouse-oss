...
| Working branch | `codex/w02-adversarial-boundary` |
| Git state | This candidate is based on protected `main` commit `aca0097ff74cc46f46e3e1b3ff4c766a5aa7d6f2`; every job in post-merge Required CI run `29877756902` passed |
...
| Active package | W02 implementation: clean-room value-free public domain validation and cross-platform local/file-URI path redaction after the merged egress, time, domain, config, identity, privacy, key-material, stable-error, and structured-event foundations |
...
| W02 — domain, config, identity, privacy | G01 | **In progress** | Merged PRs #8-#18 provide deterministic bytes/IDs, strict config/schema/CLI foundations, canonical record envelopes, keyed pseudonyms, safe URL/path/evidence handling, layered redaction, nested allowlists, secure runtime paths and secret loading, secure pseudonym-key material creation/recovery, injected UTC/monotonic clocks, an internal caller-bounded duration parser, shared stable coded errors, code-only unknown-error normalization, sink-agnostic structured events, bounded value-free config diagnostics, and the fail-closed common egress matrix. This clean-room candidate addresses the remaining known cross-surface adversarial gaps: supported concrete identity/record validation entry points retain only a fixed error type/message with no rejected value or nested exception graph, preserve exact nested Milhouse model composition without coercing foreign lookalikes or subclasses, detach UTC timestamps, and refuse caller weakening, repeated initialization, mutation, or pickle state APIs. Redaction policy `r2` keyed-pseudonymizes marked and one/two-layer percent-encoded POSIX/network/tilde/relative/Windows/UNC/file-URI paths; rejects ambiguous boundaries value-free; removes canonical-equivalent and nested percent/JSON/HTML/base64/hex registered-secret forms even beside malformed percent/hex input or MIME whitespace after an outer codec; rechecks generated pseudonyms for collisions; covers unbracketed IPv6 and reviewed filesystem-root segments at every URL path position; and uses a near-linear marked-wrapper index at the input ceiling. The synthetic corpus exercises nested, encoded, Unicode, multiline, Markdown/HTML, URL, path, PII, prompt-injection, exception, finalized-record, rendered-output, idempotence, and scaling cases. Downstream storage/runtime/CLI/report/MCP/notification packages must invoke the egress guard and obey its returned shape. W06 owns public CLI/JSON/stderr rendering; W02 retains persisted structured-log destination and rotation work pending a complete stored-log contract; W03 owns durable data-retention execution. Plugin validation, cross-process/platform identity evidence, and persisted-log contract disposition remain required before `Passed` | None beyond review |
