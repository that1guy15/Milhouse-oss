...
| Working branch | `codex/w03-slice5b2-ledger` (this PR; records the W03 slice-5b part-2 merge — the destructive `retention_apply` and its secure removal primitive, completing slice 5b) |
| Git state | Protected `main` is `a7e0f96121ebf89af7c274dc2e1b007868467310`, the verified squash merge of PR #62 (W03 slice 5b part 2 — the destructive `retention_apply` and the secure `remove_regular_file_no_follow` primitive) onto `5841eec` (PR #60, slice 5b part 1). Slice 5b (retention) is now complete; slice 5 splits into 5a `spool verify` (`f37be0d`), 5b retention (`5841eec` + `a7e0f96`), and the remaining 5c audited compaction. `retention_apply` prunes fully-expired committed segments under `barrier.exclusive()` (barrier + spool-root bound to the state root) with a confirm token, row-first (one transaction deletes the ledger + exporter rows and records the audit row, then the durable file is unlinked) so a crash leaves a re-registerable orphan file, never a row-without-file. PR #62's **destructive core passed an independent pre-merge exact-head review with no P0/P1** (verified row-first ordering, atomic rollback, only-fully-expired pruning, barrier binding/exclusivity, the secure no-follow unlink, and the crash-convergence via reconciliation re-registration); its P2 (spool-root not bound to the state root → silent no-op) and a P3 (preview/apply divergence on ledger-disagreeing segments) were fixed in-branch. It merged with green pre-merge Required CI [30551938974](https://github.com/that1guy15/Milhouse-oss/actions/runs/30551938974) and post-merge Required CI [30552197604](https://github.com/that1guy15/Milhouse-oss/actions/runs/30552197604). G02 remains accepted; G03 remains in progress, pending slice-5c audited compaction and slice-7 crash/concurrency/failure-injection and supported-host evidence |
...
| W03 — SQLite, spool, replay, retention | G02 | **In progress** | Slices 1–3d merged in PRs #38–#44 through protected `main` `075d986`: secured SQLite control state, ordered migrations, fenced leases, the global commit barrier, self-describing spool segments, atomic writer and durable ledger commit, secure bounded reads, mandatory reconciliation, and quarantine/recovery hardening. PR #44 exact head passed independent review with no actionable findings and green hosted checks before the verified merge. PR #45's structured-log persistence tree merged at protected `main` `454736f` without an exact-head PASS; a post-merge exact-head review found five P1 defects (D04), all corrected and merged by PR #54 (`1855d36`) after an independent exact-head PASS review and green Required CI. Slice 4 merged by PR #56 (`b8fff87`) after an independent pre-merge exact-head PASS: source cursors advanced only against committed segments (step 9), per-rule/version derivation checkpoints (step 10), the exporter delivery protocol with a terminal-`delivered` state machine (steps 11–12), and deterministic idempotent replay that re-reads and cross-checks each durable segment against its ledger row before reprocessing. Concrete exporters (W04), derivers (W08/W11), and the per-record query index (W09) remain deferred. Slice 5 (retention + audit + compaction) is split into 5a/5b/5c; 5a — the read-only `spool verify` audit that re-validates every committed segment against its ledger row (full field agreement + delivery watermark) without mutating and reports rather than aborting on a bad segment — merged by PR #58 (`f37be0d`), and 5b part 1 — the append-only `_audit` control table (privacy-safe, transaction-scoped, monotonic ids) and the read-only `retention_preview` that classifies each committed segment by record-class expiry (fully-expired/mixed/live/unreadable) and reports exact reclaimable counts and undelivered-expired warnings — merged by PR #60 (`5841eec`). Slice 5b is completed by part 2 — the destructive `retention_apply`, which prunes fully-expired committed segments under `barrier.exclusive()` (barrier + spool root bound to the state root) with a confirm token, row-first (the ledger + exporter rows and the audit row commit in one transaction before the durable file is unlinked via a new secure no-follow removal primitive) so a crash leaves a re-registerable orphan rather than a row-without-file, pruning only segments that agree with their ledger row and are fully expired — merged by PR #62 (`a7e0f96`) after an independent pre-merge exact-head review of the destructive core returned no P0/P1. Audited compaction (5c) remains. G03 still requires every durable-write kill point, concurrency, torn-tail recovery, persisted health and verification, replay of 10,000 records twice, retention/privacy-expiry behavior, and supported-host physical crash evidence | Independent exact-head review after each remediation; access to a supported local filesystem/host if not available to engineering |
