...
| Working branch | `codex/w02-identity-portability` |
| Git state | This candidate is based on protected `main` commit `15eb96dc23fb25b7787f1f6d3c7563a9ccd525cf`; every job in post-merge Required CI run `29899395434` passed |
...
| Active package | W02 implementation: deterministic identity portability conformance across independent processes, supported Python versions, Ubuntu, and macOS after the merged plugin/configuration boundary |
...
| W02 — domain, config, identity, privacy | G01 | **In progress** | Merged PRs #8-#20 provide deterministic bytes/IDs, strict config/schema/CLI foundations, canonical record envelopes, keyed pseudonyms, safe URL/path/evidence handling, layered redaction and allowlists, secure runtime paths and secret loading, pseudonym-key material creation/recovery, injected clocks and bounded durations, stable coded errors, sink-agnostic structured events, the fail-closed common egress matrix, adversarial domain/redaction boundaries, and metadata-only exact third-party plugin validation. This candidate adds a public-safe immutable identity corpus and independent-process recomputation under varied mapping order, hash seed, timezone, and locale. Existing required Ubuntu 24.04/Python 3.11-3.14 jobs exercise the contract, and a new aggregate-required fixed `macos-14` job exercises Python 3.11 and 3.14. Golden values cover canonical identity bytes, record IDs, dedupe keys, content hashes, and finalized record identity without network or private inputs. The same work repairs strict raw-JSON draft parsing when the optional correlation object is omitted and its exact validated default is created after JSON decoding. W02 retains persisted structured-log destination and rotation work: plan section 8.0 requires a numbered amendment before the currently unstated stored wire, rotation, crash, concurrency, and retention handoff may be implemented. W03 and W04 remain dependency-blocked until G02 passes. Hosted macOS evidence, independent review, and the stored-log amendment/disposition remain required before `Passed` | Stored-log amendment requires owner approval before implementation |
