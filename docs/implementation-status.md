...
| Working branch | `codex/w02-egress-policy` |
| Git state | This candidate is based on protected `main` commit `8f01b07c21569d87c362dcc6e038f7e68061255d`; every job in post-merge Required CI run `29876250740` passed |
...
| Active package | W02 implementation: clean-room common egress authorization, required content dispositions, and value-safe URL parser failures after the merged time, domain, config, identity, privacy, key-material, stable-error, and structured-event foundations |
...
| W02 — domain, config, identity, privacy | G01 | **In progress** | Merged PRs #8-#17 provide deterministic bytes/IDs, strict config/schema/CLI foundations, canonical record envelopes, keyed pseudonyms, safe URL/path/evidence handling, layered redaction, nested allowlists, secure runtime paths and secret loading, secure pseudonym-key material creation/recovery, injected UTC/monotonic clocks, an internal caller-bounded duration parser, shared stable coded errors, code-only unknown-error normalization, sink-agnostic structured events restricted to safe machine metadata, private raw Pydantic models, and bounded value-free config diagnostics. This clean-room candidate adds a fail-closed common surface/classification matrix with explicit independent external enablement, non-escalating allowlists, permanent restricted denial, and mandatory redacted-record, policy-filtered-summary, or metadata dispositions. It also prevents malformed URL parser failures from retaining rejected port values in exception graphs. Downstream storage/runtime/CLI/report/MCP/notification packages must invoke the guard and obey its returned shape. W06 owns public CLI/JSON/stderr rendering; W02 retains persisted structured-log destination and rotation work pending a complete stored-log contract; W03 owns durable data-retention execution. Plugin validation, cross-process/platform identity evidence, persisted-log contract disposition, and the final cross-surface G02 adversarial corpus remain required before `Passed` | None beyond review |
