...
| Working branch | `codex/w02-time-primitives` |
| Git state | This snapshot is based on protected `main` commit `2445f10b3492f6b60fd3ec7eaad80acdff4d917a`; every job in post-merge Required CI run `29867563098` passed |
...
| Active package | W02 implementation: injected UTC/monotonic clocks and an internal caller-bounded elapsed-duration parser after the merged domain, config, identity, privacy, and key-material foundations |
...
| W02 — domain, config, identity, privacy | G01 | **In progress** | Merged PRs #8-#15 provide deterministic bytes/IDs, strict config/schema/CLI foundations, canonical record envelopes, keyed pseudonyms, safe URL/path/evidence handling, layered redaction, nested allowlists, secure runtime paths and secret loading, and secure pseudonym-key material creation/recovery. This candidate adds separately injectable UTC wall and process-local monotonic readings plus a strict internal ASCII elapsed-duration parser with mandatory caller bounds and value-free stable failures. W06 owns any public CLI duration syntax, W09 owns public query windows, W15 owns scheduler/DST behavior, and W07 owns receiver secret-expiry integration. Structured logging/errors, common egress enforcement, plugin validation, cross-platform identity evidence, and the final G02 adversarial corpus remain required before `Passed` | None beyond review |
