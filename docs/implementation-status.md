...
| Working branch | `codex/w03-slice4-ledger` (this PR; records the W03 slice-4 merge — source cursors, derivation checkpoints, the exporter delivery protocol, and replay) |
| Git state | Protected `main` is `b8fff875f08aef3b8ab7a30b407b1f69767cc382`, the verified squash merge of PR #56 (W03 slice 4 — source cursors, per-rule/version derivation checkpoints, the exporter delivery protocol, and deterministic idempotent replay; pipeline steps 9–12) onto `0c2d4991` (PR #55, which recorded the D04 remediation in this ledger and advanced the G03 disposition). PR #56 passed an independent pre-merge exact-head review of its full diff — runtime logic verified against every invariant (fail-closed normalization/privacy, compare-and-set correctness, the cursor→committed-segment invariant, the terminal-`delivered` exporter state machine, and replay integrity/traversal safety) — which also caught one merge-blocker (the two guard-test updates had been left uncommitted) and two hardening nits, all corrected in-branch before merge. It merged with green pre-merge Required CI [30506867027](https://github.com/that1guy15/Milhouse-oss/actions/runs/30506867027) and post-merge Required CI [30507026824](https://github.com/that1guy15/Milhouse-oss/actions/runs/30507026824). Earlier D04 history: PR #45's structured-log surface merged at `454736f` without an exact-head PASS; a post-merge review reproduced five P1 defects, all corrected and merged by PR #54 (`1855d36`) after an independent exact-head PASS (recorded in the D04 amendment row below). G02 remains accepted; G03 remains in progress, pending the remaining W03 slice-5 retention/compaction and slice-7 crash/concurrency/failure-injection and supported-host evidence |
