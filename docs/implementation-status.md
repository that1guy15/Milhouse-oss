...
| Working branch | `codex/w02-plugin-validation` |
| Git state | This candidate is based on protected `main` commit `421effcacc43e4c4a4c72182d9cb6d12b68e3102`; every job in post-merge Required CI run `29895152754` passed |
...
| Active package | W02 implementation: clean-room, metadata-only exact third-party plugin allowlist validation after the merged adversarial domain/redaction, egress, time, config, identity, privacy, key-material, stable-error, and structured-event foundations |
...
| W02 — domain, config, identity, privacy | G01 | **In progress** | Merged PRs #8-#19 provide deterministic bytes/IDs, strict config/schema/CLI foundations, canonical record envelopes, keyed pseudonyms, safe URL/path/evidence handling, layered redaction, nested allowlists, secure runtime paths and secret loading, secure pseudonym-key material creation/recovery, injected UTC/monotonic clocks, an internal caller-bounded duration parser, shared stable coded errors, code-only unknown-error normalization, sink-agnostic structured events, bounded value-free config diagnostics, the fail-closed common egress matrix, and the adversarial concrete-domain/redaction boundary. This clean-room candidate adds the remaining metadata-only plugin configuration boundary: disabled or empty policy performs zero distribution reads; an enabled allowlist is capped at 128 entries; candidate discovery stops once ambiguity is proven; only path-backed distribution metadata is supported; and `METADATA`/`PKG-INFO` plus `entry_points.txt` are read under respective 128 KiB and 64 KiB pre-parse caps. Unsupported metadata backends fail closed, and one bounded snapshot per configured distribution is reused across its allowlist entries. Acceptance requires one exact raw distribution name, group, and syntactically valid dotted `module:attribute` value; both configured and installed raw versions must be valid PEP 440, including epochs, before exact comparison. Unlisted distributions are never inventoried, no plugin code is loaded, metadata failures retain no hostile exception graph or configured value, and W05 must revalidate and bind the exact entry-point object it immediately loads. Downstream storage/runtime/CLI/report/MCP/notification packages must invoke the egress guard and obey its returned shape. W06 owns public CLI/JSON/stderr rendering; W05 owns the public runtime registry, plugin contract kit, and `plugins` command implementation; W02 retains persisted structured-log destination and rotation work pending a complete stored-log contract; W03 owns durable data-retention execution. Cross-process/platform identity evidence and persisted-log contract disposition remain required before `Passed` | None beyond review |
